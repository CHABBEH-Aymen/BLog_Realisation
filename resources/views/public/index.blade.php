...
<div class="bg-gradient-to-r from-blue-50 via-white to-blue-50 shadow-lg rounded-lg p-8 mx-6 my-6">
  <form action="{{ route('articles.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-3 gap-6 items-end">
      <!-- Category Filter -->
  ...
  @forelse ($articles as $article)
      <div class="bg-white shadow-lg rounded-lg overflow-hidden transition-transform transform hover:-translate-y-2 hover:shadow-2xl duration-300">
          ...
          <div class="p-6">
              ...
              <!-- Article Title -->
              <a href="{{ route('public.show', $article->id) }}" class="block text-2xl font-semibold text-gray-800 hover:text-purple-600 transition-colors duration-300">
                  {{ $article->title }}
              </a>
